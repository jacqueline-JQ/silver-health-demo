...
(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory();else root.Conversation=factory();})(typeof window==='undefined'?globalThis:window,()=>{
  ...
  function parsePlan(text,old) {
    ...
    else if(explicit.length){set('slots',/(?:改为|改成|时段为|仅|只在)(?:早餐|午餐|晚餐|睡前)/.test(clean)?explicit:[...new Set([...d.slots,...explicit])]);}
    const removed=explicit.filter(slot=>new RegExp(`(?:取消|去掉|不选|不要)(?:${slot})(?:时段|这次|这一顿|服用|吃药)?(?:[，,。；;]|$)|${slot}(?:时段)?(?:取消|去掉|不选|不要吃药)(?:[，,。；;]|$)`).test(clean));
    if(removed.length)set('slots',d.slots.filter(s=>!removed.includes(s)));
    const mentions=[...clean.matchAll(/早餐|午餐|晚餐|睡前/g)],segments=mentions.map((m,i)=>({slot:m[0],text:clean.slice(m.index,mentions[i+1]?.index??clean.length)}));
    for(const {slot,text:segment} of segments){const time=segment.match(/(\d{1,2})[:：](\d{2})/);if(time){d.slotSettings[slot].time=`${time[1].padStart(2,'0')}:${time[2]}`;changed.push(`time-${slot}`);}}
