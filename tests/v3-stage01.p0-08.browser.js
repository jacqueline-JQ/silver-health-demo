async(page,{artifactDir='output/playwright/mvp-v3-stage-01'}={})=>{
  ...
  const setClock=async(offset,time)=>{await page.locator('#apply-clock').evaluate((button,{offset,time})=>{document.querySelector('#day-offset').value=String(offset);document.querySelector('#clock-time').value=time;button.click();},{offset,time});await frame.waitForFunction(({offset,time})=>{const data=JSON.parse(localStorage.getItem('silver-health-data-v1'));return data.dateOffset===offset&&data.demoTime===time;},{offset,time});};
  ...
  let early=await noticesFor(first.id,'early');ok(early.length===2&&early.every(notice=>notice.deliveryState==='pending'&&notice.deliveryAttempts===0&&notice.sentAt===null),'离线提前轮仅排队且没有伪造发送证据');
  ...
  await setClock(2,'09:00');let queued=(await noticesFor(third.id,'early')).find(notice=>notice.recipientId==='elder-zhang');const queuedId=queued.id;ok(queued.deliveryState==='pending','离线延后跨过提前时刻只保留待发轮次');
  await setMode('online');queued=(await noticesFor(third.id,'early')).find(notice=>notice.id===queuedId);ok(queued.deliveryState==='pending'&&queued.deliveryAttempts===0,'延后保护内恢复在线仍不送达');
