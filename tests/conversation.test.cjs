...
test('剂量否定改口按作用域解析',()=>{const d=draft();d.doseValue=.5;d.doseUnit='片';assert.equal(C.parsePlan('每次1片，不是2片',d).draft.doseValue,1);assert.equal(C.parsePlan('不是一片，是半片',d).draft.doseValue,.5);assert.equal(C.parsePlan('一片改成半片',d).draft.doseValue,.5);});
test('剂量否定缺新值与冲突最终值阻止核对',()=>{const d=draft();d.doseValue=.5;d.doseUnit='片';const negative=C.parsePlan('不是2片',d),conflict=C.parsePlan('每次1片，每次2片',d);assert.equal(negative.blocked,true);assert.equal(negative.blockKind,'dose');assert.deepEqual(negative.draft,d);assert.equal(conflict.blocked,true);assert.equal(conflict.blockKind,'dose');assert.deepEqual(conflict.draft,d);});
...
test('孤立餐时否定保持全部原字段',()=>{const d=draft();d.slots=['早餐','晚餐'];d.slotSettings.早餐.meal='餐后';const r=C.parsePlan('早餐不要餐前',d);assert.deepEqual(r.draft,d);assert.equal(r.blocked,true);assert.equal(r.blockKind,'meal');assert(r.issues.length);assert.deepEqual(C.parsePlan('早餐不要餐前，晚餐改为餐后',d).draft,d);});
test('餐时改口确定替换且冲突最终值阻止',()=>{const d=draft();d.slots=['早餐'];d.slotSettings.早餐.meal='餐前';const changed=C.parsePlan('早餐饭前改饭后',d),conflict=C.parsePlan('早餐改为餐前，改为餐后',d);assert.equal(changed.draft.slotSettings.早餐.meal,'餐后');assert(changed.resolvedKinds.includes('meal'));assert.equal(conflict.blocked,true);assert.deepEqual(conflict.draft,d);});
