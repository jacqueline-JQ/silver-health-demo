async(page,{artifactDir='output/playwright/mvp-v3-stage-01/migration'}={})=>{
  ...
  ok(await page.getByRole('alertdialog').count()===1&&await page.locator('[data-field="time-早餐"].field-invalid').count()===1&&await page.locator('[data-field="time-早餐"] .field-error').innerText().then(text=>/早餐用药提醒必须早于固定 N2 提前预警 09:00/.test(text))&&await page.evaluate(()=>JSON.parse(localStorage.getItem('silver-health-data-v1')).medicationPlans.length)===planCount,'新计划 N1 不早于固定 N2 时阻止保存且不写数据');await click('dismiss-errors');ok((await page.locator('[data-slot="早餐"][data-part="hour"]').innerText()).includes('09')&& (await page.locator('[data-slot="早餐"][data-part="minute"]').innerText()).includes('17'),'冲突提示后保留用户 09:17 N1 草稿');await click('close-modal');
