async (page,{artifactDir='output/playwright/step-02'}={}) => {
  ...
  const switchTo=async(id,expectedDraft)=>{await click('accounts');await page.locator(`[data-action="switch-account"][data-id="${id}"]`).click();await dismissFocus();ok(await page.locator('.nav-item.is-active[data-page="home"]').count()===1&&await page.getByRole('dialog').count()===0,`切换${id}先落首页且不自动弹草稿`);if(expectedDraft){ok(await page.locator('[data-action="resume-draft"]').count()===1,`显示${id}的继续输入入口`);await click('resume-draft');ok(await page.locator(`#modal-root [data-form="${expectedDraft}"]`).count()===1,`显式恢复${id}的${expectedDraft}草稿`);await page.locator('#modal-root [data-action="close-modal"]').click();await nav('home');}};
  ...
  await page.locator('[data-action="time-picker"][data-slot="早餐"][data-part="hour"]').click();await field('hour','8');await field('minute','47');await page.getByRole('button',{name:'确认提醒时间',exact:true}).click();
  ok((await page.locator('[data-slot="早餐"][data-part="hour"]').innerText()).includes('08'),'小时分钟输入回显');
  ...
  ok(Number(await page.locator('[name="minute"]').inputValue())!==47,'真实滚动分钟滚轮');await click('cancel-picker');
  ok((await page.locator('[data-slot="早餐"][data-part="minute"]').innerText()).includes('47'),'取消滚轮保留原值');
  ...
  const reviewText=await page.getByRole('dialog').innerText();ok(reviewText.includes('0.5 小包')&&reviewText.includes('08:47')&&reviewText.includes('餐前')&&reviewText.includes('餐后')&&reviewText.includes('2026-09-15'),'完整计划核对内容');await click('edit-med');ok(await page.locator('[name="name"]').inputValue()==='测试药 <安全>','返回修改保留草稿');await review();await page.locator('[data-action="save-med"]').evaluate(button=>{button.click();button.click();});
  ...
  await nav('home');const task=saved.doseEvents.find(e=>e.planId===plan.id&&e.slot==='早餐');await page.locator(`[data-action="take"][data-id="${task.id}"]`).click();await click('undo');await clock('11:00');await click('overdue');await page.locator(`[data-action="declare"][data-id="${task.id}"]`).click();await click('confirm-dose');
  await click('history');await page.locator(`[data-action="correct-dose"][data-id="${task.id}"]`).click();await page.locator('[data-status="taken"][data-action="choose-correction"]').click();await click('confirm-dose');await page.reload();ok((await state()).doseEvents.find(e=>e.id===task.id).status==='taken','新表单后重走打卡撤销更正与刷新');
  await clock('21:15');await click('home-add-health');await page.locator('[name="healthType"]').selectOption('身体指标');await field('height','160');await field('weight','60');await page.getByRole('button',{name:'核对测量记录',exact:true}).click();ok(await page.getByText('23.4',{exact:true}).count()===1,'BMI核对自动23.4');await click('save-health');await page.reload();await dismissFocus();ok(await page.locator('.nav-item.is-active[data-page="home"]').count()===1,'保存身体数据后刷新先回首页');await nav('health');await page.locator('[data-action="health-tab"][data-value="body"]').click();ok(await page.getByText('23.4',{exact:true}).count()>=1,'BMI保存和历史显示');
