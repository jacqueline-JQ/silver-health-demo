async(page,{artifactDir='output/playwright/mvp-v3-stage-01'}={})=>{
  ...
  await click('close-modal');await send('药名改为草稿修改药，单位换成粒，早餐提醒时间改为08:45');
  await send('时段改为早餐和晚餐');await send('晚餐提醒时间改为18:45，晚餐改为餐后');await send('开始日期改为2026-09-14，周期改为截至2026-09-20，备注改为晚饭后核对');
  const summary=await page.locator('.chat-message.assistant').last().innerText();
  ok(summary.includes('草稿修改药')&&summary.includes('每次 1 粒')&&summary.includes('早餐 08:45')&&summary.includes('晚餐 18:45 餐后')&&summary.includes('截至 2026-09-20')&&summary.includes('备注 晚饭后核对'),'聊天摘要同步全部草稿修改');
  ...
  ok((await page.locator('[data-slot="早餐"][data-part="hour"]').innerText()).includes('08')&&(await page.locator('[data-slot="晚餐"][data-part="minute"]').innerText()).includes('45'),'聊天配置卡同步两处 N1 时间');
  ...
  await page.locator('[data-form="medicine"] button[type="submit"]').click();const review=await page.getByRole('dialog').innerText();ok(review.includes('草稿修改药')&&review.includes('1 粒')&&review.includes('08:45')&&review.includes('18:45')&&review.includes('餐后')&&review.includes('2026-09-20')&&review.includes('晚饭后核对'),'最终核对页与聊天和手动表单一致');
