...
(async()=>{
  ...
  const artifactDir=path.join(path.dirname(destination),path.basename(destination,'.json'));fs.mkdirSync(artifactDir,{recursive:true});
  ...
  try{const test=vm.runInThisContext(fs.readFileSync(script,'utf8'),{filename:script});const result=await test(page,{artifactDir,handleDialogs:false,theme:process.env.TEST_THEME||'ios'});if(!result||!result.passed||result.errors?.length||dialogErrors.length)throw Error('检查函数未返回完整通过结果');fs.writeFileSync(destination,JSON.stringify(result,null,2)+'\n');console.log(`PASS ${result.passed} checks → ${destination}`);}
  catch(error){const failureBase=destination.replace(/\.json$/,'');await page.screenshot({path:`${failureBase}.failed.png`}).catch(()=>{});const contextInfo=await page.evaluate(()=>({url:location.href,dialog:document.querySelector('#modal-root')?.innerText})).catch(()=>({url:page.url()}));fs.writeFileSync(`${failureBase}.failed.json`,JSON.stringify({error:error.stack,...contextInfo},null,2));console.error(error.stack,contextInfo);process.exitCode=1;}
