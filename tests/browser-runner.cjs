...
(async()=>{
  ...
  try{const test=vm.runInThisContext(fs.readFileSync(script,'utf8'),{filename:script});const result=await test(page,{artifactDir});if(!result||!result.passed||result.errors?.length||dialogErrors.length)throw Error('检查函数未返回完整通过结果');fs.writeFileSync(destination,JSON.stringify(result,null,2)+'\n');console.log(`PASS ${result.passed} checks → ${destination}`);}
